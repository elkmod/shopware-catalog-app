<template>
  <div class="py-6 mx-6">
    <div class="flex max-w-md bg-white shadow-lg rounded-lg overflow-hidden">
      <div class="w-1/3 bg-cover bg-center" :style="style">
      </div> 
      <div class="w-2/3 p-4">
        <h1 class="text-gray-900 font-bold text-2xl">{{ product.translated.name }}</h1>
        ...
        <div class="flex item-center justify-between mt-3">
          <h1 class="text-gray-700 font-bold text-xl">{{ product.calculatedListingPrice.from.totalPrice }} €</h1>
...
export default {
  ...
  setup({ product }, {}) {
    const style = computed(() => {
      return `background-image: url('${product.cover.media.url}')`
    })
